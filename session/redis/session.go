...
// Session represents a Redis-backed session. It encapsulates the logic for accessing and manipulating
// session data stored in a Redis database.
type Session struct {
	...
	modified bool          // Flag to track whether the session has been modified
	maxAge   int           // Maximum lifetime of the session in seconds
}
...
//
// Parameters:
//   - ctx context.Context: The context in which the deletion operation is requested.
//     This context can be used to control the timeout or cancellation of the Redis
//     operation.
//   - key string: The key to remove from the session data.
//
// Returns:
//   - error: An error if the deletion operation fails, such as when the Redis
//     connection is lost; otherwise, nil is returned to indicate success.
//
// This method is safe for concurrent use by multiple goroutines, as Redis operations
// are atomic.
func (s *Session) Delete(ctx context.Context, key string) error {
	// Determine the full Redis key by combining the session key with the sub-key
	fullKey := fmt.Sprintf("%s:%s", s.key, key)

	// Delete the field from Redis
	err := s.client.Del(ctx, fullKey).Err()
	if err != nil {
		return fmt.Errorf("failed to delete key from session: %w", err)
	}
...
//
// This method adjusts the expiration time of the session in Redis.
func (s *Session) SetMaxAge(seconds int) {
	s.maxAge = seconds
	s.modified = true
}

// Save ensures that any changes made to the session are persisted to Redis.
// For Redis-backed sessions, this includes setting or updating the expiration
// time based on the maxAge value.
//
// Returns:
//   - error: An error if the save operation fails, such as when the Redis
//     connection is lost; otherwise, nil is returned.
//
// This method is safe for concurrent use by multiple goroutines.
func (s *Session) Save() error {
	// Use a background context for the Redis operation
	ctx := context.Background()

	// If maxAge is set, update the expiration time
	if s.maxAge > 0 {
		// Set a positive expiration time
		err := s.client.Expire(ctx, s.key, time.Duration(s.maxAge)*time.Second).Err()
		if err != nil {
			return fmt.Errorf("failed to set session expiration: %w", err)
		}
	} else if s.maxAge == 0 {
		// maxAge of 0 means delete immediately
		err := s.client.Del(ctx, s.key).Err()
		if err != nil {
			return fmt.Errorf("failed to delete session: %w", err)
		}
	}
	// For negative maxAge, we don't change the expiration
...
// GC performs garbage collection on expired sessions in Redis.
// Since Redis automatically removes keys that have expired, this method
// doesn't need to do any explicit cleanup. However, it can be used to
// perform any additional maintenance operations if needed.
//
// Parameters:
//   - ctx context.Context: The context in which the garbage collection is requested.
//     This context can be used to control timeout or cancellation of maintenance
//     operations.
//
// Returns:
//   - error: An error if any maintenance operations fail; otherwise, nil
//     indicating success.
//
// Note that Redis handles expiration of keys automatically, so this method
// may not be necessary for basic session management.
func (s *Store) GC(ctx context.Context) error {
	// Redis automatically removes expired keys, so no explicit GC is needed
	// However, we could potentially run a script to find and remove abandoned
	// sessions or perform other maintenance operations if needed
	return nil
}
